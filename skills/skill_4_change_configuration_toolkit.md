...
Write to: `LAR/CR-XXXX-impact.yaml`
...
Write to: `LAR/BL-[type]-[number].yaml`

After establishing the baseline, append to `LAR/cm_records.yaml`:
```yaml
...
At any time, generate a summary and append to `LAR/cm_records.yaml`:
